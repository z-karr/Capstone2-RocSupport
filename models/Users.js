...
class Users {
     ...
     */
    static async register({ username, email, password, type, address, phone }) {
        ...
        const addressRes = await db.query(
            ...
            [address.street_address, address.apartment_number, address.city, address.state, address.postal_code]
        );
        ...
        // Insert the user record into the users table, including the address and phone IDs
        const result = await db.query(
            `INSERT INTO users (
                username, 
                email, 
            ...
            [username, email, hashedPassword, type, addressID, phoneID],
        );
        const user = result.rows[0];
    ...
    static async getUserById(user_id) {
        ...
        if (!user) throw new NotFoundError(`No user found with User Id: ${user_id}`);
    ...
    static async updateByUserId(user_id, data) {
        ...
        if (data.address) {
            ...
            const { street_address, apartment_number, city, state, postal_code } = data.address;
        ...
        const addressSetCols = addressID ? `"addressID"=$${userValues.lengthn + 1}` : '';
        const phoneSetCols = phoneID ? `"phoneID"=$${userValues.length + (addressID ? 2 : 1)}` : '';

        // Generate SQL SET clause for updating username
        const usernameSetCol = username ? `"username"=$${userValues.length + (addressID && phoneID ? 3 : addressID || phoneID ? 2 : 1)}` : '';

        // Combine all SET clauses
        const setCols = [userSetCols, addressSetCols, phoneSetCols, usernameSetCol].filter(Boolean).join(', ');
        ...
        if (!user) throw new BadRequestError(`No user found with username ${username}`);
